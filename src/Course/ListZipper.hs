...
toList ::
  ListZipper a
  -> List a
toList (ListZipper l v r) = reverse l ++ (v :. Nil) ++ r
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft f (ListZipper l v r) = case fr of
  Nil -> IsNotZ
  _ -> IsZ (ListZipper (tail fr) (head fr) (reverse fl ++ (v :. r)))
  where fl = (fst . break f) l
        fr = (snd . break f) l
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight f (ListZipper l v r) = case fr of
  Nil -> IsNotZ
  _ -> IsZ (ListZipper (reverse fl ++ (v :. l)) (head fr) (tail fr))
  where fl = (fst . break f) r
        fr = (snd . break f) r
...
-- [3,2,1] >4< []
head = P.head . hlist
tail = listh . P.tail . hlist

moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop z@(ListZipper l v r) = case hasLeft z of
  True -> ListZipper (tail l) (head l) (v :. r)
  False -> ListZipper (((tail . reverse) r) ++ (v :. Nil)) ((head . reverse) r) Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop z@(ListZipper l v r) =  case hasRight z of
  True -> ListZipper (v :. l) (head r) (tail r)
  False -> ListZipper Nil ((head . reverse) l) (((tail . reverse) l) ++ (v :. Nil))
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft z@(ListZipper l v r) = case hasLeft z of
  False -> IsNotZ
  True -> IsZ $ ListZipper (tail l) (head l) (v :. r)
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight z@(ListZipper l v r) = case hasRight z of
  False -> IsNotZ
  True -> IsZ $ ListZipper (v :. l) (head r) (tail r)
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft z@(ListZipper l v r) = case hasLeft z of
  False -> IsNotZ
  True -> IsZ $ ListZipper (v :. tail l) (head l) r
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight z@(ListZipper l v r) = case hasRight z of
  False -> IsNotZ
  True -> IsZ $ ListZipper l (head r) (v :. tail r)
...
moveLeftN n z
  | n==0 = IsZ z
  | n > 0 = case asMaybeZipper moveLeft (IsZ z) of
      IsNotZ -> IsNotZ
      (IsZ z1) -> moveLeftN (n-1) z1
  | otherwise = case asMaybeZipper moveRight (IsZ z) of
      IsNotZ -> IsNotZ
      (IsZ z2) -> moveLeftN (n+1) z2
  -- | n>0 = moveLeftN (n-1) (moveLeftLoop z)
  -- | otherwise = moveLeftN (n+1) (moveRightLoop z)
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n z@(ListZipper l _ _) = go e
  where go (Right x) = IsZ x
        go (Left _) = IsNotZ
        e = moveLeftN' (length l - n) z
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end z = case hasRight z of
  False -> z
  True -> end (moveRightLoop z)
...
start ::
  ListZipper a
  -> ListZipper a
start z = case hasLeft z of
  False -> z
  True -> start (moveLeftLoop z)
...
deletePullLeft (ListZipper l _ r) = IsZ $ ListZipper (tail l) (head l) r
...
deletePullRight (ListZipper l _ r) = IsZ $ ListZipper l (head r) (tail r)
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft x (ListZipper l v r) =
  ListZipper (v :. l) x r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight x (ListZipper l v r) =
  ListZipper l x (v :. r)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper fl fv fr) (ListZipper l v r) =
    ListZipper (zipWith go fl l) (fv v) (zipWith go fr r)
    where go x = x
    -- error "todo: Course.ListZipper (<*>)#instance ListZipper"
...
instance Extend ListZipper where
  (<<=) f lz = ListZipper l x r
    where l = unfoldr (g moveLeft) lz
          x = f lz
          r = unfoldr (g moveRight) lz
          g h y = case h y of
            IsNotZ -> Empty
            _ -> (,)
                 <$> (f <$> (toOptional . h) y)
...
instance Extend MaybeListZipper where
  (<<=) _ IsNotZ = IsNotZ
  (<<=) f m@(IsZ z) = IsZ $ ListZipper l x r
    where
      l = unfoldr (g moveLeft) z
      x = f m
      r = unfoldr (g moveRight) z
      g h y = case h y of
        IsNotZ -> Empty
        _ -> (,)
             <$> Full ((f . h) y)
