...
(|||) ::
  ...
  -> Parser a
(|||) p1 p2 = P f
  where f s = case (parse p1) s of
          Result ss x -> Result ss x
          _ -> (parse p2) s
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser b
  (=<<) f p = P g
    where g s = case parse p s of
            Result ss x -> parse (f x) ss
            _ -> UnexpectedString s
    -- error "todo: Course.Parser (=<<)#instance Parser"
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser b
  (<*>) = (<**>)
    -- remember reader also a monad, but why?? @notes, need check more
    -- error "todo: Course.Parser (<*>)#instance Parser"
...
satisfy ::
  ...
  -> Parser Char
satisfy f = P g
  where g Nil = UnexpectedEof
        g (x :. xs) = case f x of
          True -> Result xs x
          False -> UnexpectedChar x
...
-- digit = foldRight ((|||) . is) (pure 'a') (listh ['0' .. '9'])
...
sequenceParser' (p :. ps) = P f
  where f s = case parse p s of
          Result ss x -> g x <$> parse (sequenceParser' ps) ss
          _ -> UnexpectedString s
        g y z = y :. z
              --g y z@(Result i w) = Result i (y :. w)
              --g y _ = UnexpectedString s
  -- error "todo: Course.Parser#sequenceParser"
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser Nil = pure Nil
sequenceParser (p :. ps) = go =<< p
  where go x = f <$> (sequenceParser ps)
          where f y = x :. y
...
firstNameParser = (:.) <$> (
  -- (\k -> case isUpper k of
  --     False -> constantParser (UnexpectedChar k)
  --     True -> pure k)
  pure =<< upper
  ) <*> list lower
...
phoneParser ::
  Parser Chars
phoneParser = --(++) <$> g <*> ((:. Nil) <$> (is '#'))
   g <* (is '#')
  where g = (:.)  <$> (pure =<< digit ) <*> phoneBodyParser
...
personParser =
  Person <$>
  ageParser <*>~ firstNameParser <*>~ surnameParser <*>~
