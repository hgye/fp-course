...
-- Empty
-- getOptional :: Optional a
-- getOptional Empty = Empty
...
seqOptional (e :. es ) = case e of
  Empty -> Empty
  Full i -> f P.<$> (Full i) P.<*> seqOptional es
        where f x y = x :. y
